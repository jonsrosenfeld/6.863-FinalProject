...
def get_args():
    ...
    parser.add_argument('--save_path', type=str, default='results_P2')
...
# NEW DATASET
  if dataset_type=="aaaa_train_dev":
     ...
     train, dev, test = data.TabularDataset.splits(path='', format='tsv', train="Datasets/different_size_train_grammar2/grammar2_train-15000.tsv", validation="Datasets/grammar2_dev.tsv", test="Datasets/different_size_train_grammar2/grammar2_train-15000.tsv", fields=[('text', TEXT), ('label', LABELS)])  #
     ...
     #train_iter, val_iter, train_retest_iter = data.BucketIterator.splits((train, dev,test), batch_size=50, sort_key=lambda x: len(x.text), device=args.gpu)
           ...
           # correct2=0
            for dev_batch_idx, dev_batch in enumerate(val_iter):
                ...
                # print (dev_batch)
                 ...
                 #print (F.softmax(answer).data)
                 #_, predicted = torch.max(F.softmax(answer).data, 1)
                 #print(predicted)
                 #correct2  +=(predicted == dev_batch.label.data).sum()
                
           # print ("ACC RECHECK")
           # total = len(dev)
           # accuracy = float(100 * correct2.item() / total)
           # print (accuracy)
