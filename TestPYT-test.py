...
#space_sep_to_space_sep("merge_train_first.txt","train_main.tsv")
#space_sep_to_space_sep("merge_test_final.txt","dev_main.tsv")
...
def get_args():
    ...
    parser.add_argument('--d_embed', type=int, default=50) #300 128 20 10
    parser.add_argument('--d_proj', type=int, default=300)
    parser.add_argument('--d_hidden', type=int, default=20)  # 128 100 168
...
def main_loop_structure(dataset_type, train_adress="", dev_adress="", test_adress=""): #  "aaaa_train_dev", "SST.train_dev_test" split
  args = get_args()
...
# NEW DATASET
  if dataset_type=="aaaa_train_dev":
     ...
     #train, dev = data.TabularDataset.splits(path='', format='tsv', train="train_main.tsv", validation="dev_main.tsv", fields=[('text', TEXT), ('label', LABELS)]) 
     print ("Dataset_split done")  # out_train.tsv  out_dev.tsv
     ...
     #train_iter, val_iter = data.BucketIterator.splits((train, dev), batch_size=50, sort_key=lambda x: len(x.text), device=0)
     train_iter, val_iter, train_retest_iter = data.BucketIterator.splits((train, dev, test), batch_size=50, sort_key=lambda x: len(x.text), device=0)
     #train_iter, val_iter, train_retest_iter = data.BucketIterator.splits((train, dev,test), batch_size=50, device=0)
     print ("Iterators are set")
  ...
  if dataset_type=="SST_train_dev_test":
      ...
      train_iter, val_iter, test_iter = data.BucketIterator.splits((train, dev, test), batch_size=args.batch_size, device=None)
#TEXT=inputs
#LABELS=answers
...
#  print ("Weight_init_check")
#  g=model.lstm.all_weights
#  for weight in g:
#      print (weight)
  ...
  for epoch in range(args.epochs):
    ...
    for batch_idx, batch in enumerate(train_iter):
        ...
        # evaluate performance on validation set periodically
            ...
            true_train_acc=100. * n_train_correct/len(test)
...
'''
  f=open('results_with_interpretation_sem2_new.txt','a')
